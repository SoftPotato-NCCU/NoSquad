...
describe('t() - translation function', () => {
  ...
  it('returns translation for valid nested key', () => {
    expect(t(mockDict, 'auth.login.title', 'Fallback')).toBe('Login to NoSquad');
  });
  ...
  it('returns fallback when value is not a string', () => {
    const dictWithObject = { auth: { login: { form: { submit: { text: 'Login' } } } } };
    expect(t(dictWithObject, 'auth.login.form.submit', 'Fallback')).toBe('Fallback');
  });
