...
describe('api.ts', () => {
  ...
  describe('isAuthError', () => {
    it('returns true for UNAUTHORIZED error', () => {
      const error = { error: { code: 'UNAUTHORIZED', message: 'Token missing' } };
      expect(isAuthError(error)).toBe(true);
    });

    it('returns true for INVALID_TOKEN error', () => {
      const error = { error: { code: 'INVALID_TOKEN', message: 'Invalid token' } };
      expect(isAuthError(error)).toBe(true);
    });

    it('returns true for TOKEN_EXPIRED error', () => {
      const error = { error: { code: 'TOKEN_EXPIRED', message: 'Token expired' } };
      expect(isAuthError(error)).toBe(true);
    });

    it('returns true for USER_NOT_FOUND error', () => {
      const error = { error: { code: 'USER_NOT_FOUND', message: 'User not found' } };
      expect(isAuthError(error)).toBe(true);
    });

    it('returns false for non-auth error', () => {
      const error = { error: { code: 'VALIDATION_ERROR', message: 'Invalid data' } };
