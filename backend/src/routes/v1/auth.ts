...
auth.post("/register", async (c) => {
  ...
  if (!body.password)
    ...
    !/(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[^A-Za-z0-9])/.test(body.password)
  )
    details.push({
      field: "password",
      issue: "complexity",
  ...
  if (existing.length > 0)
    return apiError(
      ...
      409,
      "USER_EXISTS",
      "One or more credentials (email, username, or phone) are already in use",
    );
...
auth.post("/login", async (c) => {
  ...
  if (
    !user ||
    !(await Bun.password.verify(body.password, user.hashed_password))
  )
    return apiError(
      c,
      401,
  ...
  await pool.execute("UPDATE users SET last_activity = NOW() WHERE uuid = ?", [
    user.uuid,
  ]);
